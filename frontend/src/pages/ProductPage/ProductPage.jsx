...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      setError("");
      console.log(
        `Fetching product details for ID: ${id} from ${PRODUCTS_API_BASE_URL}/${id}`
      );
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch product details: ${response.statusText} (${response.status})`
          );
        }
  ...
  const handleAddToWishList = async () => {
    ...
    const token = localStorage.getItem("token");
    console.log("TOKEN przed fetch:", token);
    if (!token) {
      console.log(
        "JWT token not found in localStorage for wishlist, redirecting to login."
      );
      navigate("/login");
      return;
    }
    ...
    <ProductContainer>
      ...
      <ProductDetails>
        ...
        <ProductPrice>
          ...
          {product.type === "coffee"
            ? product.prices && product.prices[selectedSizeIndex]
